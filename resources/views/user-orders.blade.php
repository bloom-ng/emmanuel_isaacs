            ...
            <li class="mb-8 shadow-xl">
                <div class="border-t border-gray-200">
                    <dl>
                        ...
                        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 font-semibold sm:col-span-2">
                                &#8358;{{ number_format($order->sub_total + $order->shipping_total, 2) }} (Subtotal:
                                &#8358;{{ $order->sub_total }} | Shipping: &#8358;{{ $order->shipping_total }})
                            </dd>
                    ...
                    <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                        <button
                            @click="expandedOrder === {{ $order->id }} ? expandedOrder = null : expandedOrder = {{ $order->id }}"
                        ...
                        <div class="mt-4 divide-y divide-gray-200">
                            ...
                            <div class="flex items-center py-4">
                                <img src="{{ asset('storage/'.$item->product->image) }}"
                                    alt="{{ $item->product->name }}" class="h-16 w-16 rounded-md shadow-md">
                                <div class="ml-4">
                                    ...
                                    <a download="" href="{{ asset('storage/'.$item->product->download_link) }}"
                                        class="inline-block mt-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700">Download</a>
                                    @endif
                                </div>
                            </div>
                            @endforeach
